...
import { SPECIES, isObserved, PLAYERS, ALL_PLAYERS } from './data'
...
export function Territory({ wide, lang, onBack, edit }) {
  ...
  useEffect(()=>{ try { const r = localStorage.getItem('pluduni_pins'); if(r) setCustom(JSON.parse(r)) } catch(e){} },[])
  const savePins = (next)=>{ setCustom(next); try { localStorage.setItem('pluduni_pins', JSON.stringify(next)) } catch(e){} }
  ...
  return (
    <div style={{ padding: wide?'16px 40px 40px':'14px 18px 30px' }}>
          ...
          {sel && (
            <div style={{ position:'absolute', left:10, right:10, bottom:10, background:'rgba(20,22,14,.93)',
              ...
              <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom:3 }}>
                ...
                {edit && sel.custom && (
                  <button onClick={()=>{ savePins(custom.filter(x=>x.id!==sel.id)); setSel(null) }}
      ...
      {draft && <PinEditor draft={draft} lang={lang} onCancel={()=>setDraft(null)}
        onSave={(p)=>{ savePins([...custom, { ...p, id:'c'+Date.now(), custom:true }]); setDraft(null) }} />}
    </div>
  )
}
