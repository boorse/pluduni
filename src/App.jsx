...
import { splitInds, promote, demote, getOverride, loadOverrides, subscribe } from './individuals.js'
...
export default function App() {
  ...
  useEffect(() => {
    loadOverrides().then(()=>setRefresh(r=>r+1))
      ...
      return (
        <div style={{ padding:'10px 14px 26px' }}>
          ...
          <div style={{ background:T.surface, borderRadius:16, border:`1px solid ${T.line}`, overflow:'hidden' }}>
            {mobileTab==='map' ? <MindMap onSelectSpecies={selSpFull} lang={lang} expanded={mapExpanded} setExpanded={setMapExpanded} tf={mapTf} setTf={setMapTf} /> : <MatrixPane compact />}
          </div>
        </div>
      )
    }
      ...
      <div style={{ padding:'12px 24px 28px', display:'flex', gap:12, alignItems:'stretch', height:'calc(100vh - 132px)', minHeight:520 }}>
        <div onMouseEnter={()=>setFocus('map')} onClick={()=>setFocus('map')}
          ...
          <div style={{ flex:1, overflow:'hidden' }}>
            <MindMap onSelectSpecies={selSpFull} lang={lang} expanded={mapExpanded} setExpanded={setMapExpanded} tf={mapTf} setTf={setMapTf} />
          </div>
        </div>
