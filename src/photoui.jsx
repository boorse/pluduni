...
// ── Hook : photos d'une cible, avec URLs objets ──
export function usePhotos(target) {
  const [photos, setPhotos] = useState([])
  const urls = useRef([])
  const load = useCallback(async () => {
    if (!target) return
    const list = await getPhotos(target)
    urls.current.forEach(u => URL.revokeObjectURL(u))
    urls.current = list.map(p => URL.createObjectURL(p.blob))
    setPhotos(list.map((p, i) => ({ ...p, url: urls.current[i] })))
  }, [target])
      ...
      <div onClick={e=>e.stopPropagation()} style={{ background:T.bg, borderRadius:18, width:'100%',
        ...
        <div style={{ padding:'14px 18px 20px' }}>
            ...
            : <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(120px,1fr))', gap:9 }}>
                {photos.map(p=>(
                  <div key={p.id} style={{ position:'relative', borderRadius:11, overflow:'hidden',
                    ...
                    <button onClick={async()=>{ await deletePhoto(p.id); reload() }}
          ...
          <div style={{ marginTop:14, fontSize:11, color:T.mute, lineHeight:1.5, display:'flex', gap:6 }}>
            ...
            <span>{lang==='ru'
              ? 'Фото хранятся в этом браузере. Совместный доступ появится позже.'
              : 'Les photos sont stockées dans ce navigateur, compressées à 1600 px. Le partage entre vous quatre viendra ensuite.'}</span>
          </div>
        </div>
      </div>
    </div>
