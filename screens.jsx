...
import { SPECIES, isObserved, PLAYERS } from './data'
...
export function Calendar({ wide, lang, onBack }) {
  ...
  const [who, setWho] = useState(PLAYERS[3].name)
  useEffect(()=>{ try { const r = localStorage.getItem('pluduni_todos'); if(r) setTodos(JSON.parse(r)) } catch(e){} },[])
  const save = (next) => { setTodos(next); try { localStorage.setItem('pluduni_todos', JSON.stringify(next)) } catch(e){} }
  const add = () => { if(!draft.trim()) return; save([...todos, { id:Date.now(), txt:draft.trim(), by:who, done:false, theme }]); setDraft('') }
  const toggle = (id) => save(todos.map(x=>x.id===id?{...x,done:!x.done}:x))
  const del = (id) => save(todos.filter(x=>x.id!==id))
...
// ══════════ TERRITOIRE — Google Maps satellite ══════════
export function Territory({ wide, lang, onBack }) {
  const t = UI[lang]
  const [sel, setSel] = useState(null)
  const [filter, setFilter] = useState('all')
  const list = POIS.filter(p => filter==='all' || p.t===filter)
  const CENTER = { lat:57.3128, lon:25.2741 }
  const target = sel?.gps ? { lat:sel.gps[0], lon:sel.gps[1] } : CENTER
  const zoom = sel ? 17 : 15
  const src = `https://maps.google.com/maps?q=${target.lat},${target.lon}&t=k&z=${zoom}&ie=UTF8&iwloc=&output=embed`
  ...
  return (
    <div style={{ padding: wide?'16px 40px 40px':'14px 18px 30px' }}>
      ...
      <p style={{ fontSize:12.5, color:T.mute, marginBottom:14 }}>
        {lang==='ru'?'Точки интереса на спутниковой карте':'Points d\u2019intérêt sur la vue satellite'}
      </p>
      ...
      <div style={{ display:'grid', gridTemplateColumns: wide?'1.6fr 1fr':'1fr', gap:14 }}>
        <div style={{ borderRadius:16, overflow:'hidden', border:`1px solid ${T.line}`, position:'relative' }}>
          <iframe title="Carte satellite" src={src} loading="lazy"
            style={{ width:'100%', height: wide?520:320, border:'none', display:'block' }} />
          {sel && (
            <div style={{ position:'absolute', left:10, right:10, bottom:10, background:'rgba(43,38,32,.93)',
              ...
              {sel.gps && <div style={{ fontSize:10.5, color:'rgba(242,238,226,.55)', marginTop:4 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7, maxHeight: wide?520:'none', overflowY:'auto' }}>
          {list.map(p=>{
            const ty = POI_TYPES[p.t]; const on = sel?.id===p.id
            return (
...
export function Gallery({ wide, lang, onBack }) {
  ...
  const shots = []
  SPECIES.filter(isObserved).forEach(sp => (sp.inds||[]).forEach(ind => shots.push({ sp, ind })))
  ...
  return (
    <div style={{ padding: wide?'16px 40px 40px':'14px 18px 30px' }}>
      ...
      <p style={{ fontSize:12.5, color:T.mute, marginBottom:16 }}>
        {shots.length} {lang==='ru'?'снимков особей':'clichés d\u2019individus'}
      </p>
      <div style={{ display:'grid', gridTemplateColumns:`repeat(auto-fill,minmax(${wide?170:140}px,1fr))`, gap:10 }}>
        {shots.map(({sp,ind},i)=>(
          <button key={i} onClick={()=>setBox({sp,ind})} style={{ textAlign:'left', borderRadius:14,
            ...
            <div style={{ position:'relative', height:'100%', display:'flex', flexDirection:'column', justifyContent:'space-between', padding:11 }}>
              ...
              <div>
                ...
                <div style={{ fontSize:10.5, color:'rgba(242,238,226,.78)', marginTop:2 }}>{nameOf(sp,lang).main}</div>
      ...
      {box && <Lightbox {...box} lang={lang} wide={wide} onClose={()=>setBox(null)} />}
    </div>
  )
}

function Lightbox({ sp, ind, lang, wide, onClose }) {
  return (
    <div onClick={onClose} style={{ position:'fixed', inset:0, background:'rgba(20,18,14,.92)', zIndex:100,
      ...
      <div onClick={e=>e.stopPropagation()} style={{ position:'relative', width:'100%', maxWidth:760,
        maxHeight:'92vh', borderRadius: wide?18:0, overflow:'hidden', display:'flex', flexDirection:'column' }}>
        <div style={{ position:'relative', flex:1, minHeight: wide?460:340, background:gradientFor(sp.id+ind.n),
          ...
          <button onClick={onClose} style={{ position:'absolute', top:14, right:14, width:34, height:34,
            borderRadius:'50%', background:'rgba(0,0,0,.4)', color:'#fff', fontSize:16,
          ...
          <div style={{ position:'absolute', top:14, left:16 }}>
            ...
            <div className="serif" style={{ fontSize: wide?26:21, fontWeight:900, color:'#F2EEE2', lineHeight:1.05 }}>{ind.n}</div>
          </div>
          <div style={{ position:'absolute', left:0, right:0, bottom:0,
            background:'linear-gradient(to top, rgba(14,16,10,.9), rgba(14,16,10,.55) 55%, transparent)',
            padding: wide?'34px 22px 20px':'28px 16px 16px' }}>
            {ind.story && (
            ...
            {mySpecies.map(sp=>{
              ...
              return (
                <button key={sp.id} onClick={()=>onSelectSpecies(sp.id)} style={{ textAlign:'left', borderRadius:12,
                  ...
                  <div style={{ position:'absolute', inset:0, background:gradientFor(sp.id) }} />
